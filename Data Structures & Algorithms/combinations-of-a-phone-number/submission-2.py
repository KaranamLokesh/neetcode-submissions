class Solution:
    ...
    def check (self, digits, res, digitToChar):
        n = len(digits)   
        ## recursive case
        ## the idea is to keep on adding one digit at a time,
        ## res will be empty at first, then add first digit it will have list of first digit
        ## then we keep on adding the new digit to result, maintaining the same list 
        ## 
        for digit in digits:
