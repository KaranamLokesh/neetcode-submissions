class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        if i < len(height) - 1:
            # only do this if you dont find a large element to the right side, left > right, so we have to reverse traverse
            ...
            while j >= 0:
                if height[j] > height[i]:
                    # no need to check = as we have already calculated in the first pass
